...
class Game(QMainWindow):
    def __init__(self, parent=None):
        ...
        # Определим цвет плитки каждого номинала
    ...
    def initGameData(self):
        ...
        while count < 2:
            ...
            col = randint(0, len(self.data[0]) - 1)
            if self.data[row][col] != 0:
                continue
            self.data[row][col] = 2 if randint(0, 1) else 4
            count += 1
        self.curScore = 0
        self.bstScore = 0
        # Читаем из файла максимальный рекорд
    ...
    def drawTiles(self, qp):
        ...
        for row in range(4):
            for col in range(4):
                ...
                # Условие: если значение больше 0, то выводим текст
                if value != 0:
                    # print("[%d][%d]: value[%d]" % (row, col, value))
                    rect = QRect(30 + col * 115, 165 + row * 115, 100, 100)
                    # Установка цвета текста для чисел 2 и 4
                    if value == 2 or value == 4:
                        qp.setPen(QColor(119, 110, 101))  # Тёмный цвет текста для маленьких номеров
                    else:
                        qp.setPen(QColor(255, 255, 255))  # Белый текст для больших номеров
                    
                    # Отображаем текст в центре плитки
    ...
    def drawScore(self, qp):
        ...
        qp.drawText(scoerLabelRect, Qt.AlignmentFlag.AlignCenter, u"SCORE")
        qp.setPen(QColor(255, 255, 255))
    ...
    def slideUpDown(self, isUp):
        ...
        for col in range(numCols):
            ...
            for i in range(numRows - len(cvl)):
                if isUp:
                    cvl.append(0)  # Добавляем нули в конец для движения вверх
                else:
                    cvl.insert(0, 0)  # Добавляем нули в начало для движения вниз
